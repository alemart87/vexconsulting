...
/** Agente Cowork: el compañero de equipo IA del proyecto.
 *
 ...
 *  @mención se invita a un compañero (campana con link directo).
 *
 *  Vive en dos formas (patrón de los copilots pro — panel lateral de
 *  320-480px junto al trabajo, plegable):
 *   - variante PÁGINA (/agent): hilos a la izquierda + chat a la derecha
 *   - variante DOCK: panel lateral derecho disponible en TODO el proyecto,
 *     para conversar con el agente mientras se mira el documento, el Gantt
 *     o un flow. Los hilos se abren como vista propia dentro del panel.
 */
...
export default function CoworkAgent({
  dock = false,
  onClose,
}: {
  /** true = panel lateral compacto (una columna, hilos como vista interna) */
  dock?: boolean;
  /** dock: plegar el panel */
  onClose?: () => void;
}) {
  ...
  // dock: alternar entre el chat y la lista de hilos
  ...
  /* ---- Lista de conversaciones (compartida entre página y dock) ---- */
  const convList = (
    <>
      {convs.length === 0 && (
  ...
  const archivedToggle = (archivedCount > 0 || showArchived) && (
    <button
  ...
  /* ---- Hilo: mensajes + composer (compartido entre página y dock) ---- */
  const thread = (
    <>
  ...
  /* ================== Variante DOCK (panel lateral) ================== */
  if (dock) {
    return (
      <div className="flex flex-col h-full min-h-0">
        <div className="px-3.5 py-2.5 border-b border-brand-border flex items-center gap-2.5 bg-brand-bg-soft/60 shrink-0">
          <AgentAvatar active={thinking} />
          <div className="min-w-0 flex-1">
            <div className="text-[13px] font-bold text-brand-ink truncate leading-tight">
              {view === "threads" ? "Hilos del equipo" : activeConv?.title || "Agente Cowork"}
            </div>
            <div className="text-[10.5px] text-brand-slate truncate">
              {view === "threads"
                ? "Compartidos con todo el proyecto"
                : activeConv && activeConv.participants.length > 1
                  ? `con ${activeConv.participants.join(" y ")}`
                  : "Compañero IA · conoce el documento"}
            </div>
          </div>
          <div className="flex items-center gap-0.5 shrink-0">
            <button
              title={view === "threads" ? "Volver al chat" : "Hilos del equipo"}
              className={`h-7 w-7 rounded-md text-[13px] flex items-center justify-center transition-colors ${
                view === "threads"
            ...
            <button
              ...
              className="h-7 w-7 rounded-md text-[15px] text-brand-slate hover:bg-white hover:text-brand-ink flex items-center justify-center transition-colors"
              onClick={newConversation}
            >
              +
            </button>
            <Link
              href={`/projects/${params.id}/agent`}
              title="Abrir en pantalla completa"
              className="h-7 w-7 rounded-md text-[12px] text-brand-slate hover:bg-white hover:text-brand-ink flex items-center justify-center transition-colors"
            >
              ⤢
            </Link>
            {onClose && (
              <button
                ...
                className="h-7 w-7 rounded-md text-[13px] text-brand-slate hover:bg-white hover:text-brand-ink flex items-center justify-center transition-colors"
                onClick={onClose}
              >
                ⟩
              </button>
  ...
  return (
    <div className="grid lg:grid-cols-[300px_1fr] gap-4 items-start">
      ...
      <div className="card overflow-hidden">
        ...
        <div className="max-h-[62vh] overflow-y-auto scrollbar-thin">{convList}</div>
        {archivedToggle}
      </div>
