...
Cómo trabajás:
- En esta conversación puede haber VARIAS personas del equipo. Cada mensaje indica \
quién habla; dirigite por su nombre cuando ayude, y si alguien fue mencionado para \
sumarse, dale contexto breve de lo conversado antes de responder.
...
async def list_conversations(
    ...
    convs = (await db.execute(
        ...
        ).order_by(Conversation.updated_at.desc()).limit(40)
    )).scalars().all()
    out = []
    for c in convs:
...
async def send_message(
    ...
    system = _SYSTEM.format(project_name=access.project.name, document=doc_md[:MAX_DOC_CHARS])
